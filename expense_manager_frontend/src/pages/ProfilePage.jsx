...
import { LogOut, Trash2, Save, User, Mail, AtSign, Wallet, AlertTriangle, X, ChevronDown } from "lucide-react";
...
function DateExpensesModal({ date, onClose }) {
    ...
    // each row ~44px, header ~65px, footer ~44px
    // target 8 rows visible = 8 * 44 = 352px body, total ~460px
    const ROW_HEIGHT = 44;
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center"
            ...
            <div className="relative flex flex-col rounded-2xl shadow-2xl"
                style={{
                    ...
                    // let it grow naturally but cap it
                    maxHeight: "90vh",
                    overflow: "hidden",
                }}
                ...
                {/* Scrollable body — capped at 8 rows */}
                <div style={{
                    overflowY: "auto",
                    maxHeight: `${bodyMaxHeight}px`,
                    // custom scrollbar
                    scrollbarWidth: "thin",
                    scrollbarColor: isDark ? "rgba(78,222,163,0.3) transparent" : "rgba(16,185,129,0.3) transparent",
                }}>
                    {loading ? (
                            ...
                            <tbody>
                                {expenses.map((exp, i) => (
                                    <tr key={exp.expenseId}
                                        style={{
                                            borderBottom: i < expenses.length - 1 ? "1px solid rgba(78,222,163,0.05)" : "none",
                ...
                {!loading && expenses.length > 0 && (
                    <div className="flex items-center justify-between px-5 py-3 shrink-0 text-xs"
                        ...
                        <span style={{ color: "var(--color-text-secondary)" }}>
                            {expenses.length} transaction{expenses.length !== 1 ? "s" : ""} · Total spent
                        </span>
...
function ActivityHeatmap({ expenses: initialExpenses }) {
    ...
    async function handleYearChange(y) {
        ...
        setLoadingYear(true);
        try {
            const data = await getExpensesByDateRange(`${y}-01-01`, `${y}-12-31`);
            setExpenses(data);
        } catch {
            setExpenses([]);
        } finally {
    ...
    return (
        <div className="bg-surface-high rounded-xl p-4 md:p-6 w-full max-w-full overflow-hidden">
                    ...
                    <button onClick={() => setYearDropOpen(o => !o)}
                        ...
                        {loadingYear ? (
                            <span className="flex items-center gap-1.5">
                                <div className="w-3 h-3 rounded-full border animate-spin" style={{ borderColor: "var(--color-primary)", borderTopColor: "transparent" }} />
                                Loading…
                            </span>
                        ) : (
            ...
            <style>{`
                ...
                .heatmap-scroll::-webkit-scrollbar-thumb { background: ${thumbColor}; border-radius: 999px; }
                .heatmap-scroll::-webkit-scrollbar-thumb:hover { background: ${thumbHoverColor}; }
            `}</style>

            <div className="heatmap-scroll pb-2"
                style={{ overflowX: "scroll", overflowY: "hidden", WebkitOverflowScrolling: "touch", touchAction: "pan-x pinch-zoom", scrollbarWidth: "thin", scrollbarColor: `${thumbColor} transparent`, cursor: "grab" }}>
                        ...
                        {weeks.map((week, wi) => {
                            ...
                            return (
                                <div key={wi} style={{ width: `${CELL}px`, flexShrink: 0, position: "relative", height: "18px" }}>
                                    {monthLabel && (
                                        <span style={{ position: "absolute", left: 0, bottom: 2, fontSize: "11px", color: isDark ? "#8892a4" : "#4A6358", whiteSpace: "nowrap", fontFamily: "monospace" }}>
                                            {monthLabel}
                                        </span>
                                    )}
                                </div>
                    ...
                    <div style={{ display: "flex", gap: "4px" }}>
                        <div style={{ display: "flex", flexDirection: "column", gap: `${GAP}px`, width: "32px", flexShrink: 0 }}>
                            {DAY_LABELS.map((label, i) => (
                                <div key={i} style={{ height: `${CELL}px`, fontSize: "11px", color: isDark ? "#8892a4" : "#4A6358", display: "flex", alignItems: "center", justifyContent: "flex-end", paddingRight: "4px", visibility: (i === 1 || i === 3 || i === 5) ? "visible" : "hidden" }}>
                                    {label}
                                </div>
                            ))}
                        </div>
...
function ProfilePage() {
    ...
    return (
        <div className="flex flex-col gap-6 w-full max-w-full overflow-x-hidden pb-4 md:pb-0">
            ...
            <div className="bg-surface-high rounded-xl p-4 md:p-6 w-full">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6 w-full">
                <div className="bg-surface-high rounded-xl p-5 md:p-6 w-full">
                    ...
                    <div className="flex flex-col sm:flex-row gap-3 items-stretch sm:items-start w-full">
                        ...
                        <button onClick={handleSaveBudget} disabled={isSavingBudget}
                            className="w-full sm:w-auto flex justify-center items-center gap-2 px-6 py-3 rounded-xl text-sm font-medium shrink-0"
                ...
                <div className="bg-surface-high rounded-xl p-5 md:p-6 border border-error/20 w-full">
                    ...
                    {!showDeleteConfirm ? (
                        <button onClick={() => setShowDeleteConfirm(true)}
                            className="w-full sm:w-auto flex justify-center items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-medium transition-colors bg-error/10 text-error border border-error/30 hover:bg-error/20">
                        ...
                        <div className="flex flex-col gap-3 w-full">
                            ...
                            <div className="flex flex-col sm:flex-row gap-2 w-full">
                                <button onClick={handleDeleteAccount} className="w-full sm:w-auto px-4 py-2 rounded-lg text-sm font-medium bg-error text-white">
                                    {isDeleting ? "Deleting..." : "Yes, delete"}
                                </button>
                                <button onClick={() => setShowDeleteConfirm(false)} className="w-full sm:w-auto px-4 py-2 rounded-lg text-sm font-medium text-text-secondary bg-surface-low">
                                    Cancel
                                </button>
                            </div>
                        </div>
            ...
            <button onClick={logout} className="flex items-center gap-2 text-text-secondary text-sm hover:text-error transition-colors self-start mb-10 md:mb-2">
                <LogOut size={14} />
